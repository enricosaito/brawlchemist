...
function noticeFor(sp: {
  ...
  if (sp.error) {
    return {
      ...
      text:
        sp.error === "bad-cm-id"
          ? "No Challengermode id in that. Paste the competitor's profile link — challengermode.com/users/<id> — or the id itself. A username cannot be resolved: the API has no lookup by name."
          : sp.error === "cm-no-user"
            ? "Challengermode has no user with that id, so it would never match anyone in a bracket. Check the link and try again."
            : sp.error === "upload"

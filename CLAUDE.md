...
7. **The Brawlhalla API double-encodes UTF-8.** It serves `application/json` but builds its JSON escapes from already-UTF-8 bytes treated as Latin-1, so "Areté" arrives as "AretÃ©". Not a font bug — no display change fixes it. `repairJson()` (lib/text.ts) is applied once in `apiFetch`, so everything downstream is clean; it is deliberately conservative (telltale byte pattern + exact round-trip check) because an eager version corrupts legitimately non-ASCII names. 3,313 stored usernames needed repair; 14 failed the round-trip check and were left alone.
